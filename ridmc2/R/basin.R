#' @useDynLib ridmc2 ridmc_basin_alloc
#' @useDynLib ridmc2 ridmc_basin_finished
#' @useDynLib ridmc2 ridmc_basin_step
#' @useDynLib ridmc2 ridmc_basin_getData
#' @useDynLib ridmc2 ridmc_basin_slow_alloc
#' @useDynLib ridmc2 ridmc_basin_slow_setGslRngSeed
#' @useDynLib ridmc2 ridmc_basin_slow_finished
#' @useDynLib ridmc2 ridmc_basin_slow_step
#' @useDynLib ridmc2 ridmc_basin_slow_getData
#' @export
Basin <- function(model, parms, xlim, xres=100, ylim,
  ...
  if(method=="fast") {
    ...
    while(.Call("ridmc_basin_finished", basin)==0)
      .Call("ridmc_basin_step", basin)
    values <- .Call("ridmc_basin_getData", basin)
  } else {
    if(missing(ntries))
    ...
    if(!missing(seed))
      .Call("ridmc_basin_slow_setGslRngSeed", basin,
        as.integer(seed))
    while(.Call("ridmc_basin_slow_finished", basin)==0)
      .Call("ridmc_basin_slow_step", basin)
    values <- .Call("ridmc_basin_slow_getData", basin)
  }
  values <- t(values)
